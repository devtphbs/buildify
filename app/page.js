...
export default function AuthPage() {
  ...
  async function handleSignup(e) {
    e.preventDefault()
    setLoading(true); setError('')
    const { data, error } = await sb.auth.signUp({ email, password, options: { emailRedirectTo: undefined } })
    ...
    setLoading(false)
    if (e2) setError('Account created! Please sign in.')
    else router.push('/dashboard')
  }

  const inp = { width:'100%', background:'#0f0f1e', border:'1.5px solid var(--border)', borderRadius:12, padding:'12px 14px', color:'#fff', fontSize:15, marginBottom:14, fontFamily:'Nunito,sans-serif' }
  const lbl = { fontSize:11, fontWeight:800, color:'#555', letterSpacing:.8, display:'block', marginBottom:6 }

  return (
    <div style={{ minHeight:'100dvh', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', padding:'40px 24px', position:'relative', overflow:'hidden', background:'var(--dark)' }}>
      ...
      <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:24, padding:'28px 24px', width:'100%', maxWidth:360, animation:'slideUp .5s ease .1s both' }}>
        ...
        {mode === 'login' ? (
          <form onSubmit={handleLogin}>
            ...
            <button type="submit" disabled={loading} style={{ width:'100%', background:'var(--grad)', border:'none', borderRadius:12, padding:14, color:'#fff', fontWeight:800, fontSize:16, boxShadow:'0 4px 18px #FF3CAC33', opacity:loading?.6:1, cursor:loading?'default':'pointer' }}>
          ...
          <form onSubmit={handleSignup}>
            ...
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" required style={inp} />
            <label style={lbl}>PASSWORD</label>
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Min 6 characters" required minLength={6} style={{...inp, marginBottom:20}} />
            <button type="submit" disabled={loading} style={{ width:'100%', background:'var(--grad)', border:'none', borderRadius:12, padding:14, color:'#fff', fontWeight:800, fontSize:16, boxShadow:'0 4px 18px #FF3CAC33', opacity:loading?.6:1, cursor:loading?'default':'pointer' }}>
